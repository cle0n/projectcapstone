...
# Then, stop keeping track of loops until the current node diverges from the loop that was just found, to avoid duplicate loops and extra work
			...
			if node in path:  
				...
				if divergedFromLastLoop:
					...
					### This section shortens the loops! Less memory, but less path information
					shortloop = [loop[0], loop[-1]]
					if shortloop not in self.looplist:
						self.looplist.append(shortloop)
					# This is the end of the loop shortening section. 
					# Uncomment the next line and comment out this section to lengthen the loops.
